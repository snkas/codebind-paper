...
int main(int argc, char *argv[]) {
    ...
    CommandLine cmd;
    std::string run_dir = "";
    cmd.Usage("Usage: ./waf --run=\"main-full-with-protocol --run_dir='<path/to/run/directory>'\"");
    cmd.AddValue("run_dir",  "Run directory", run_dir);
    cmd.Parse(argc, argv);
    if (run_dir.compare("") == 0) {
        printf("Usage: ./waf --run=\"main-full-with-protocol --run_dir='<path/to/run/directory>'\"");
        return 1;
    }
    ...
    TcpFlowScheduler tcpFlowScheduler(basicSimulation, topology, {TcpFlowScheduler::DEFAULT_SERVER_PORT}, CreateObject<ClientRemotePortSelectorDefault>(TcpFlowScheduler::DEFAULT_SERVER_PORT), tcpSocketGeneratorCustom, CreateObject<IpTosGeneratorDefault>()); // Requires enable_tcp_flow_scheduler=true
