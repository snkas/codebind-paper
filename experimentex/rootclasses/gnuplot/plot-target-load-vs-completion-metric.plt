...
set yrange [90:]       # Explicitly set the y-range [lower:upper]
# set xtics (0, 100, 300, 500, 700, 900)
# set ytics <start>, <incr> {,<end>}
set format y "%g%%"  # Set the x-tic format, e.g. in this case it takes 2 sign. decimals: "24.13%""
set format x "%.0f%%"
...
plot    "[DATA-FILE1]" using ($1):($2*100.0):($3*100.0):($4*100.0) title "[TITLE-LINE1]" w yerrorlines ls [COLOR-LINE1], \
        "[DATA-FILE2]" using ($1):($2*100.0):($3*100.0):($4*100.0) title "[TITLE-LINE2]" w yerrorlines ls [COLOR-LINE2], \
