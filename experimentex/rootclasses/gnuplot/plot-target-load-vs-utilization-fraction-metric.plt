...
set ylabel "Max. link utilization (%)"

# Axes ranges
set xrange [0:]       # Explicitly set the x-range [lower:upper]
set yrange [0:100]       # Explicitly set the y-range [lower:upper]
...
plot    "[DATA-FILE1]" using ($1):($2*100):($3*100):($4*100) title "[TITLE-LINE1]" w yerrorlines ls [COLOR-LINE1], \
        "[DATA-FILE2]" using ($1):($2*100):($3*100):($4*100) title "[TITLE-LINE2]" w yerrorlines ls [COLOR-LINE2], \
